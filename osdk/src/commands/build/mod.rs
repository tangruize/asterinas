...
pub fn do_cached_build(
    ...
    if !config.use_cached_bundle {
        return build_a_new_one();
    }

    if let Some(existing_bundle) = get_reusable_existing_bundle(&bundle_path, config, action) {
        info!("Reusing existing bundle as requested; ensure the source code remains unchanged");
...
pub fn do_build(
    ...
    // Recheck the existing bundle's reusability if it is not checked (i.e., !config.use_cached_bundle)
    if !config.use_cached_bundle {
        if let Some(existing_bundle) = get_reusable_existing_bundle(&bundle_path, config, action) {
