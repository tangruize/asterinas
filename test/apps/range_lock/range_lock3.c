...
int main() {
    ...
    if (pid == 0) {
        ...
        acquire_lock(fd, F_WRLCK, 0, 1, "Child1");

        sleep(2);

        // Child process exits
        close(fd);
        printf("Child1 process exits\n");
        exit(EXIT_SUCCESS);
    } else {
        pid_t pid2 = fork();
        ...
        if (pid2 == 0) {
            acquire_lock(fd, F_WRLCK, 0, 1, "Child2");
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if (result == 0) {
                printf("Timeout: Child%d process did not exit within 10 seconds\n", i+1);
            } else if (result == -1) {
                perror("waitpid");
            } else {
                printf("Waitpid: Child%d process exited\n", i+1);
            }
        }
