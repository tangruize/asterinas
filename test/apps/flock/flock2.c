...
int main() {
    ...
    if (pid2 == 0)
    {
        ...
        // Try to place exclusive lock on fd3
        printf("Child2 tries to place exclusive lock placed on fd3\n");
        while (flock(fd3, LOCK_EX | LOCK_NB) < 0);

        printf("Child2: exclusive lock placed on fd3\n");

        sleep(3);
        printf("Child2 releases lock\n");
        flock(fd3, LOCK_UN);
        close(fd3);
