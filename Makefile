...
# Track dependencies and skip `cargo build` if a cached bundle is reusable;
# The `USE_CACHED_BUNDLE` option saves only 0.1s, as `cargo build` also tracks dependencies
...
kernel_src_deps: $(_KERNEL_DEPS_DUMMY_TARGET)
	$(eval $(if $(filter 0,$(_KERNEL_DEPS_CHANGED)), CARGO_OSDK_ARGS += --use-cached-bundle))
else
kernel_src_deps:
endif
